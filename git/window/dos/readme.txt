第一章 批处理基础 http://www.cnblogs.com/glaivelee/archive/2009/10/07/1578737.html
第一节 常用批处理内部命令简介
...
简单的说ping -n 10 127.1>nul 就是让程序等待N秒
...
h V InM0gl V0屏幕上就出现是否将A:盘格式化的内容。
...
{TSC1U3s9xW0　　@format X: /q/u/autoset (format 这个命令是不可以使用/y这个参数的，可喜的是微软留了个autoset这个参数给我们，效果和/y是一样的。)
...
mR5T8c0　　goto label （label是参数，指定所要转向的批处理程序中的行。） 
　　Sample： 
　　if {%1}=={} goto noparms 
...
jG&@!e"he,NPr0　　指定要调用的批处理程序的位置和名称。filename 参数必须具有 .bat 或 .cmd 扩展名。
