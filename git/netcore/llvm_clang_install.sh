...
#参考自：https://github.com/owent-utils/bash-shell/blob/master/LLVM%26Clang%20Installer/3.9/installer.sh
...
function check_and_download(){
	...
	#echo "---------check_and_download---$PKG_NAME---$PKG_MATCH_EXPR--$PKG_VAR_VAL---$PKG_URL-----"
	
    
    if [ -e "$PKG_VAR_VAL" ]; then
        echo "$PKG_VAR_VAL"
        return 0;
    fi
...
echo "-----------------2.2--------------------"
    ...
    if [ "0" == $(is_in_list llvm $BUILD_TARGET_COMPOMENTS) ]; then
        LLVM_PKG=$(check_and_download "llvm" "llvm-$LLVM_VERSION.src.tar.xz" "http://llvm.org/releases/$LLVM_VERSION/llvm-$LLVM_VERSION.src.tar.xz" );
        if [ $? -ne 0 ]; then
            echo -e "$LLVM_PKG";
            return -1;
        fi
...
echo "-----------------2.3--------------------"
    ...
    if [ "0" == $(is_in_list clang $BUILD_TARGET_COMPOMENTS) ]; then
        CLANG_PKG=$(check_and_download "clang" "cfe-$LLVM_VERSION.src.tar.xz" "http://llvm.org/releases/$LLVM_VERSION/cfe-$LLVM_VERSION.src.tar.xz" );
        if [ $? -ne 0 ]; then
            echo -e "$CLANG_PKG";
            exit -1;
        fi
...
echo "-----------------2.4--------------------"
    ...
    if [ "0" == $(is_in_list clang_tools_extra $BUILD_TARGET_COMPOMENTS) ]; then
        CLANG_TOOLS_EXTRA_PKG=$(check_and_download "clang tools extra" "clang-tools-extra-$LLVM_VERSION.src.tar.xz" "http://llvm.org/releases/$LLVM_VERSION/clang-tools-extra-$LLVM_VERSION.src.tar.xz" );
        if [ $? -ne 0 ]; then
...
echo "-----------------2.5--------------------"
    ...
    if [ "0" == $(is_in_list compiler_rt $BUILD_TARGET_COMPOMENTS) ]; then
        COMPILER_RT_PKG=$(check_and_download "compiler rt" "compiler-rt-$LLVM_VERSION.src.tar.xz" "http://llvm.org/releases/$LLVM_VERSION/compiler-rt-$LLVM_VERSION.src.tar.xz" );
        if [ $? -ne 0 ]; then
...
echo "-----------------2.6--------------------"
    ...
    if [ "0" == $(is_in_list libcxxabi $BUILD_TARGET_COMPOMENTS) ]; then
        LIBCXXABI_PKG=$(check_and_download "libc++abi" "libcxxabi-$LLVM_VERSION.src.tar.xz" "http://llvm.org/releases/$LLVM_VERSION/libcxxabi-$LLVM_VERSION.src.tar.xz" );
        if [ $? -ne 0 ]; then
...
echo "-----------------2.7--------------------"
    ...
    if [ "0" == $(is_in_list libcxx $BUILD_TARGET_COMPOMENTS) ]; then
        LIBCXX_PKG=$(check_and_download "libc++" "libcxx-$LLVM_VERSION.src.tar.xz" "http://llvm.org/releases/$LLVM_VERSION/libcxx-$LLVM_VERSION.src.tar.xz" );
        if [ $? -ne 0 ]; then
            echo -e "$LIBCXX_PKG";
            exit -1;
        fi
...
echo "-----------------2.8--------------------"
    ...
    if [ "0" == $(is_in_list lld $BUILD_TARGET_COMPOMENTS) ]; then
        LLD_PKG=$(check_and_download "lld" "lld-$LLVM_VERSION.src.tar.xz" "http://llvm.org/releases/$LLVM_VERSION/lld-$LLVM_VERSION.src.tar.xz" );
        if [ $? -ne 0 ]; then
            echo -e "$LLD_PKG";
            exit -1;
        fi
...
echo "-----------------2.9--------------------"
    ...
    if [ "0" == $(is_in_list polly $BUILD_TARGET_COMPOMENTS) ] && [ -e /usr/bin/python2 ] && [ -e "$CLANG_DIR" ]; then
        POLLY_PKG=$(check_and_download "polly" "polly-$LLVM_VERSION.src.tar.xz" "http://llvm.org/releases/$LLVM_VERSION/polly-$LLVM_VERSION.src.tar.xz" );
        if [ $? -ne 0 ]; then
            echo -e "$POLLY_PKG";
            exit -1;
        fi
...
if [ "0" == $(is_in_list lldb $BUILD_TARGET_COMPOMENTS) ]; then
    ...
    if [ 0 -eq $LLDB_DEP_PYTHON2 ] && [ 0 -eq $LLDB_DEP_SWIG ] && [ ! -z "$LLDB_DEP_LIBEDIT" ] && [ ! -z "$LLDB_DEP_LIBXML2" ] && [ ! -z "$LLDB_DEP_NCURSES" ] ; then
        LLDB_PKG=$(check_and_download "lldb" "lldb-$LLVM_VERSION.src.tar.xz" "http://llvm.org/releases/$LLVM_VERSION/lldb-$LLVM_VERSION.src.tar.xz" );
        if [ $? -ne 0 ]; then
            echo -e "$LLDB_PKG";
            exit -1;
        fi
...
if [ "0" == $(is_in_list libunwind $BUILD_TARGET_COMPOMENTS) ]; then
    LIBUNWIND_PKG=$(check_and_download "libunwind" "libunwind-$LLVM_VERSION.src.tar.xz" "http://llvm.org/releases/$LLVM_VERSION/libunwind-$LLVM_VERSION.src.tar.xz" );
    if [ $? -ne 0 ]; then
        echo -e "$LLDB_PKG";
        exit -1;
    fi
...
echo "-----------------2.12--------ready to build------------"
    ...
    # if [ ! -e "$STAGE_BUILD_PREFIX_DIR/bin/llvm-config" ]; then
		...
		pwd
		echo "-----------------begin cmake--------------------"
		echo "cmake .. -DCMAKE_INSTALL_PREFIX=$STAGE_BUILD_PREFIX_DIR -DCMAKE_BUILD_TYPE=Release $BUILD_LLVM_LLVM_OPTION $STAGE_BUILD_CMAKE_OPTION $STAGE_BUILD_EXT_CMAKE_OPTION;"
...
#echo "[ 0 -ne $? ]"
#fi
#if [ ! -e "$STAGE_BUILD_PREFIX_DIR/bin/clang" ]; then
#echo "$STAGE_BUILD_PREFIX_DIR/bin/clang not found"
#fi
#if [ 0 -ne $? ] || [ ! -e "$STAGE_BUILD_PREFIX_DIR/bin/clang" ]; then
