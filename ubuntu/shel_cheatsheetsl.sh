...
# 文件操作
##############################################################################
dir --sort=time --time-style="+%Y-%m-%d %H:%M:%S" -lah  #时间排序
ll --time-style=full-iso
dir --sort=size -lah #大小排序
 dir --sort=extension -lah # 扩展名
ls                  # 显示当前目录内容，后面可接目录名：ls {dir} 显示指定目录
ls -lth --time-style="+%Y-%m-%d %H:%M:%S" # 按照时间降序（最新修改的排在前面）-t     sort by modification time 按修改时间排序（最新的在最前面）-r     reverse order while sorting （反序）
