...
cat > /etc/vsftpd/vsftpd.conf <<efo
local_enable=YES
write_enable=YES
local_umask=022
...
cat >> /etc/vsftpd/user_list<< efo
ftpuser
efo

# 在最后一行追加刚才创建的用户名 ftpuser
...
# 如果服务器开启了防火墙，则需要配置允许21端口访问

# iptables -A INPUT -p tcp --dport 21 -j ACCEPT

# iptables -A OUTPUT -p tcp --sport 21 -j ACCEPT

# 五、在web 浏览器中访问 fpt://ip ，输入 ftpuser 用户名和密码，测试登录
